...
// uno
const int selector[6] = {17, 16, 15, 14, 0, 3}; // input pins for selector buttons (A0=14, A1=15, A2=16, A3=17, Dig0=0) UNO
...
// #max
// #PREIS
int conveyorPrice[6] = {2500, 2500, 2500, 2500, 2500, 2500}; // default price 6
int conveyorItems[6] = {1, 1, 1, 1, 1, 1};
/*
// 5 boxes
...
const int relays[6] = {4, 5, 6, 7, 8, 9}; // uno: 6 boxes
...
//
  ...
  lcd.print("Prosze czekac..."); // Print wait message to the LCD
 delay(1000);
   if (debug) {
...
case 6: coinsCurrentValue = coinsCurrentValue + 100;
        break;
case 7: coinsCurrentValue = coinsCurrentValue + 300;
        break;        
   } // switch
  } // if
...
// Im Leerlauf werden nur Textnachrichten angezeigt, wenn der Akku nicht im Energiesparmodus ist
 ...
 // #PREIS
      ...
      lcd.setCursor(0, 0);
      lcd.print("Prosze wrzucic ");
      lcd.setCursor(0, 1);
      lcd.print("odliczona kwota. ");
    } else if (randomNumber == 2) {
      lcd.setCursor(0, 0);
      lcd.print("Wrzuc kwote, pozniej");
      lcd.setCursor(0, 1);
...
lcd.clear();
           lcd.setCursor(0, 0);
           lcd.print("Schowek otwarty.");
           lcd.setCursor(0, 1);
           lcd.print("(3) nacisnij!");
           delay(200);
      } //selector [3]
...
if (debug) {
 ...
 if (debug) {
    ...
    while (scope == 0) {
      ...
      if (digitalRead(selector[1]) == LOW && conveyorPrice[index] < 10000) {
        conveyorPrice[index] = conveyorPrice[index] + 10;
        displayConfPrice(index);
        delay(300);
      }
      ...
      // Price +5
         if (digitalRead(selector[3]) == LOW && conveyorPrice[index] < 10000) {
       conveyorPrice[index] = conveyorPrice[index] - 500;
        displayConfPrice(index);
        delay(300);
      }
        if (digitalRead(selector[4]) == LOW && conveyorPrice[index] >= 10) {
        conveyorPrice[index] = conveyorPrice[index] + 500;
        displayConfPrice(index);
        delay(300);
      }
...
void displayEmpty() {
  ...
  lcd.clear(); // reset LCD
  lcd.setCursor(0, 0);
  lcd.print("Schowek pusty :(");
  lcd.setCursor(0, 1); // set cursor to LCD row 2 column 1 (starting with 0)
  lcd.print("Wybierz inny");
  if (coinsCurrentValue > 0) {
    delay(1000);
    displayBalance();
  }
